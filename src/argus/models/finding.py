...
class FindingId(BaseModel):
    ...
    def __str__(self) -> str:
        return f"argus-{self.category}-{self.file}-{self.line}"
...
class StableId(BaseModel):
    ...
    def __str__(self) -> str:
        return f"argus-{self.category}-{self.file}::{self.function_name}"
...
class Finding(BaseModel):
    ...
    def from_hypothesis(cls, hyp, func) -> Finding:
        ...
        finding_id = f"argus-{hyp.category.value}-{file_name}-{func.start_line}"
        stable_id = f"argus-{hyp.category.value}-{file_name}::{func.name}"
        return cls(
