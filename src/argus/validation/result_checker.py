...
from argus.models.core import SignalCategory
...
def check_result(
    ...
    """Check PoC execution result for success evidence."""
    checkers = {
...
def _check_memory(stdout, stderr, exit_code, http_response, sanitizer_output) -> ValidationStatus:
    """Memory corruption: ASAN/MSAN/UBSAN report."""
    if sanitizer_output:
        return ValidationStatus.CONFIRMED
    if "AddressSanitizer" in stderr or "MemorySanitizer" in stderr or "runtime error:" in stderr:
        return ValidationStatus.CONFIRMED
