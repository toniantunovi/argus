...
from prowl.config import SandboxConfig
from prowl.sandbox.images import compute_image_tag, get_dockerfile
from prowl.sandbox.instrumentation import get_compile_flags, parse_sanitizer_output
...
class DockerSandboxManager:
    ...
    async def execute_poc(
        ...
        staging_dir = Path(tempfile.mkdtemp(prefix="prowl-poc-"))
        try:
            # Copy target code
    ...
    def _build_image(self, tag: str, dockerfile_content: str, context_dir: Path) -> None:
        ...
        build_dir = Path(tempfile.mkdtemp(prefix="prowl-build-"))
        try:
