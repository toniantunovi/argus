...
from prowl.config import load_config
...
def missed(location, category, description):
    """Report a vulnerability that Prowl missed."""
...
def status():
    ...
    manager = ScanStateManager(Path(".prowl/scan-state"))
...
def findings(severity, category):
    ...
    cache_file = Path(".prowl/cache/last_report.json")
    if not cache_file.exists():
        click.echo("No findings available. Run 'prowl scan' first.")
        return
